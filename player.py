...
class Player(ShipClass):
    ...
    def __init__(self, x, y, health, ship_img, bullet_img):
        ...
        self.total_bullets_fired = 0  # Contador total de balas disparadas
    ...
    def create_bullets(self, game):
        ...
        if self.request_fire:
            # Disparos desde la parte inferior de la pantalla en la posición X del cursor
            try:
                mx, my = pygame.mouse.get_pos()
                bx = mx - (self.bullet_img.get_width() // 2 if self.bullet_img else 0)
                by = game.screen_height - (self.bullet_img.get_height() + 10 if self.bullet_img else 20)
            except Exception:
                # fallback: usar centro de la pantalla
                bx = game.screen_width // 2
                by = game.screen_height - 20

            bullet = Bullet(bx, by, self.bullet_img)
            self.fired_bullets.append(bullet)
            self.total_bullets_fired += 1  # Incrementar contador de balas disparadas
    ...
    def cooldown(self, game):
        ...
        if self.creation_cooldown_counter > 0:
            ...
            if self.creation_cooldown_counter >= self.cool_down:
                # Si usamos HUD (game.bullets no es None), sumar una bala al HUD hasta el máximo permitido
                if game.bullets is not None:
                    try:
                    ...
                    # HUD infinito: no necesitamos regenerar balas
                    self.creation_cooldown_counter = 0
    ...
    def hit(self, enemies, game):
        ...
        for bullet in self.fired_bullets[:]:
            for enemy in enemies[:]:
                # Verificar colisión entre bala y enemigo (pixel-perfect)
                try:
                    collision = bullet.collision(enemy)
                except Exception:
                    collision = None
                if collision:
                    ...
                    if enemy in enemies:
                        try:
                            ...
                            pass
                    # Registrar kill para el contador del powerup y en el HUD global
                    self._record_kill()
                    try:
                        game.enemy_hits += 1
                    except Exception:
